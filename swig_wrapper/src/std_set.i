...
%typemap(csinterfaces) std::set< V > "global::System.IDisposable \n /*   , System.Collections.Generic.ICollection<$typemap(cstype, V)>*/";
%typemap(cscode) std::set< V > %{
...
/*
  public void CopyTo( $typemap(cstype, V)[] array, int arrayIndex)
  {
    if (array == null)
    ...
    int i = 0;
    foreach($typemap(cstype, V) element in this)
    {
      array.SetValue(getitemcopy(element),i );
    i++;
    }
  }

  global::System.Collections.Generic.IEnumerator<$typemap(cstype, V)> global::System.Collections.Generic.IEnumerable<$typemap(cstype, V)>.GetEnumerator() {
  ...
  public:
    ...
    %extend {
      ...
      const value_type& getitem(const value_type& val) throw (std::out_of_range) {
       ...
       value_type getitemcopy(const value_type& val) throw (std::out_of_range) {
        std::set< V >::iterator iter = $self->find(val);
        if (iter != $self->end())
          return *iter;
        else
