...
//%define VECTOR_EXT(TYPENAME)
%extend gpstk::Vector {
   gpstk::Vector<T> gpstk::Vector(std::vector<T> input) {
      const int n = input.size();
      std::valarray<T> tmp(n);
      for (int i = 0; i < n; i++) {
         tmp[i] = input[i];
      }
     return new gpstk::Vector<T>(tmp);
   }
...
%template (gpstk_vector) gpstk::Vector< double >;
%template (gpstk_Vector_SatID) gpstk::Vector< gpstk::SatID >;
/*
%pythoncode %{
