...
/** @file Rinex3EphemerisStore.hpp Read and store RINEX formated
...
namespace gpstk
{
   ...
   private:
      ...
      FileStore<Rinex3NavHeader> NavFiles;
         ...
         /** Ephemeris store for orbit-based nav messages (stored as
          * descendents of OrbitEph), including systems GPS, Galileo,
          * BeiDou, and QZSS */
   ...
   public:
      ...
      Rinex3NavData Rdata;
         ...
         /** Map of time system corrections, similar to mapTimeCorr in
          * Rinex3NavHeader, and taken from either loadFile()
          * (RinexNavHeader) or user input.  key =
          * TimeSystemCorrection::asString4().  User may add to the
          * list with addTimeCorr() */
      ...
      int loadFile(const std::string& filename, bool dump=false,
         ...
         /** use to access the data records in the store in bulk Add
          * all Rinex3NavData in this store to the given list. If sat
          * is defined, (its default is (-1,mixed)), then add only
      ...
      * @return the number of records */
      ...
      bool delTimeCorr(const std::string& typestr)
      {
         ...
         /** Fill out the time system corrections "network" by adding
          * corrections that can be derived from existing
      ...
      int expandTimeCorrMap(void)
      {
         ...
         /** Utility routine for getXvt and addEphemeris to test time
          * systems and convert if necessary. Convert ttag to the
          * target time system, using the first appropriate correction
          * in mapTimeCorr, and return it.  If no correction is found,
          * ttag is unchanged and an exception is thrown. */
      CommonTime correctTimeSystem(const CommonTime ttag,
                                   const TimeSystem targetSys) const;

         /** Find the appropriate time system correction object in the
          * collection for the given time systems, and dump it to a
      ...
      void SearchNear(void)
      {
         ...
         /** use findEphemeris() in the getSat...() routines (the
          * default) (Orbits) */
